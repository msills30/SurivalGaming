...
#preload loads the material immediately while load 'loads' the material when activated
...
@onready var constructable_collision_shape: CollisionShape3D = $ConstructableArea/CollisionShape3D
...
func _ready() -> void:
	...
	constructable_collision_shape.shape = constructable_preview_mesh.mesh.create_convex_shape()
...
func _process(_delta: float) -> void:
	...
	set_value(check_build_validity())

func set_value(valid:bool) -> void:
